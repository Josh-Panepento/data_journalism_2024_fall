...
**Task** Run the following code and describe the dataframe it outputs. **Answer** It put the "releases" data into the environment. When you click on it, there are hundreds of rows.
...
**Task** Run the following code and describe what the `unique` function does to the original list of words. **Answer** It showed all the unique words and got rid of the duplicates. Like it just puts out one "dog" even though there are two in the list.
...
**Task** Run the following code and describe the output of using the `unnest_tokens` function. **Answer** It opened a table with one column that lists all the unique words. There are so many.
...
**Task** Run the following code and describe what the resulting graphic shows. Is it interesting? **Answer** It shows which unique words are the most common. For example, the top word is "the", followed by "and".
...
**Task** Run the following code and describe the results. Is it more interesting than before? **Answer** It is much more interesting. It shows a table with the percentage of the word used and it is not showing the short "filler" words.
...
**Task** Run the following code and describe the results. **Answer** It gave two tables with the most common words in the years 2022 and 2024. It shows the amount of times used and the percentage.
...
**Task** Run the following code and describe the results. **Answer** It is a table that shows the most common two words that are used together like "covid 19" and "health care". It shows the top ten from the year 2022.
...
**Task** Run the following code and describe the results. **Answer** It is similar to the last one but now it is the top 10 two word phrases from 2024. We see that "health care" is now the most popular.
...
**Task** Run the following code and describe the differences between the two results. Which one might be more useful for the press release data? **Answer** The top line shows more words in the past tense while the bottom has "drive" written five times. Since the top has more unique words, I would use that one.
...
**Task** Run the following code and describe the differences between the two results. Which one is more useful? **Answer** They are both from 2024 but the first table (lemma) is taking the word to its base so "fund" is more common because to took "funding", etc. I think this one is more useful because it shows how often a word is used in a more effective way.
...
**Task** Run the following code and describe the results. Do any of the sentiments seem incorrect or counter-intuitive? **Answer** It is showing word counts from 2022 and 2024 but putting "positive" or "negative" next to the words. At first look none seem too counter-intuitive but there are a few that could go the other way depending on the context.
