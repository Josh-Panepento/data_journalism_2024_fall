...
**Task** Export the CSV file to your pre_lab_06 folder and read it into R by running the following codeblock. What problems do you see with the data? **Answer** There's a ;pt pf N/A's and the totals are used as the header instead of being its own thing so there's not a traditional header.
...
**Task** Get rid of rows we don't need by running the codeblock below. What problems remain? **Answer** The header is still messed up and does not properly label anything. It's different than before but still wrong.
...
**Task** Run the codeblock below. What changed, and what issues or problems remain? **Answer** It looks much better, the headers actually tell what is going on below and there are new columns for party, office, etc. It is much easier to read.
