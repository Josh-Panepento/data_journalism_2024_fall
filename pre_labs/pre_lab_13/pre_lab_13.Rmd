...
```{r}
create_credentials(GROQ_API_KEY = "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")
```
...
**Answer** The first ten seem to have a theme of fun and. playful while the back half is more about "strong".
...
**Answer** It was a bit inconsistent. It pulled names but it also pulled "Mass", "31" and "33".
...
**Answer** I find it interesting that it created a new csv file beacuse it thought there were so many job requirements. If you wanted to avoid this, I would change the prompt to speicifically say to make just one csv.
...
**Answer** It says in order for it to read the pipe, I need to install the package "magrittr". This does not make sense to me because we have not used that library but it has been able to read those pipe operators.
...
**Answer** The output does make sense and would help understand the data but it could use some extra things to help it. For example it could suggest cleaning the names and I would also have to verify if the column names are the same and stuff like that.
...
**Answer** The extract column seems to do well in identifying mentions of "merchandise that can be won," but there were still some holes. I'm not entriely sure it caught the correct things.
