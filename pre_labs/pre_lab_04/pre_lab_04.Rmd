...
**Task** Run the following code to load some campaign expenditure data focused on food-related expenses in Washington, D.C. **Answer** I'm not sure what I'm supposed to be answering
...
**Task** Run the following code to group and count records in the recipient_name column, and filter to examine only recipients that start with W. Name a problem that you see that is preventing proper grouping. **Answer** There are many "W. Millar & Co. Catering" but they are all spelled and classified differently.
...
**Answer** There are a lot of "Capitol Hill Club" entries which I'm assuming we just cleaned so they are together. It also took "Scarlet Oak" and made it lowercase.
...
**Task** Run the following code to group by recipient_clean and recipient_name, then filter for cities that start with W. Name three changes the refinr function made in the first 20 results. **Answer** It took W. Millar & Co. and put those together. It also made another one called "W. Millar & Co. Catering" although those seem related they are two different groups. It also put companies like Walgreens and corrected the different misspellings.
...
**Task** Run the following code to use the n_gram_merge() function to clean the data, then group by city_clean and city to examine cities that start with A. Look at the differences. Does the changes it made vary substantially from the previous use of key_collision_merge? **Answer** Similar to the last one, it attempted to clean the spelling or capitalization of the cities but still had some flaws.
