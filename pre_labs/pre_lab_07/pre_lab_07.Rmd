...
```{r echo=FALSE}
census_api_key("dcdc08efb218907b709505abf4624bd34d830b49", install=TRUE)
```
...
**Task** Run the following codeblock to calculate the percentage change and put the state with the largest percentage growth below. **Answer** Nevada has the largest with a 35.1 percent increase.
...
**Task** Run the following codeblock to find the Maryland county with the highest median household income and put the answer below. **Answer** Howard county has the highest median household income at 140,971 dollars.
...
Is the difference between Calvert and Montgomery significant? **Answer** The bars overlap so the differences aren't statistically significant.

Is the difference between Howard and everyone else significant? **Answer** Howard's bar does not overlap with anyone so it is significant.
...
**Answer** Most of the zip codes belonged to Montgomery County which had a median income well above $50k. The rest belonged to Anne Arundel, Baltimore county and city, Kent, and Talbot which also made over 50k.
...
**Task** Arrange md_zips_with_acs by estimate showing the highest median household income first. What questions do you have about these top zip codes? **Answer** I would like to know which county these zip codes belong to, how many people live in these zip codes, and what these people do for work?
