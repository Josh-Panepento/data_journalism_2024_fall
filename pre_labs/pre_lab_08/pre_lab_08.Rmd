...
**Answer** The first line is assigning md_voters. The second line is mutating a category for unaffiliated difference by subtracting unaffiliated in 2024 from unaffiliated in 2022. The third is assigning md_voters to maryland_top_counties. The next line is having them appear in descending order. The next is showing just the columns for counties and unaffiliated difference and the next line is to show the top 10. Finally, the last line is to display the results.
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** It is creating a blank canvas to display the chart.
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** It is a bar graph showing the difference in unaffiliated voters. There are 10 bars for each county, all in gray.
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer** Now it is in order so the biggest change is first and so on. The pattern makes a curve and looks much cleaner.
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer** It turned so the bars are going horizontal instead of vertical and they are still in order of largest change to smallest.
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer** It took away the gray background and now it is white.
...
**Task** Run the following code. **Answer** The code installed and loaded the library "ggthemes".
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last version of the chart? **Answer** The background is now a light blue but the lines are going with the bars now instead of marking where the numbers are.
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer** It now has a title on the top and the source of the info on the bottom.
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** It shows a table with columns for "date" and "total_contributions". There are 91 rows.
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** There is a line chart with total_contributions on the side and date on the bottom. It shows the day-by-day income of contribution money.
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** It made it so it has specific dates instead of just months.
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** It reoriented the text for the dates so it is slanted and easier to read.
