...
**Answer** It says, "Warning: One or more parsing issues, call `problems()` on your data frame for details"
...
**Answer** There are 1,640 rows. I think it could be having problems interpreting the data which could be a problem with the data like we talked about in class.
...
**Answer** This time, it did not have a warning or problems.

```{r}
texas_precinct_20 <- read_csv("data/tx_precinct_2020.csv", guess_max=476915) ##guess max is the number of rows
```
...
**Answer** District and mail are doubles which means they have numeric values with decimal points. Precinct is character which means it has text.
...
texas_precinct_20 <- texas_precinct_20 |>
  mutate(election_day = case_when(
    election_day == '5+' ~ '49',
    TRUE ~ election_day ##This keeps everything else the same
  ))
...
**Answer** There are over 400 thousand that say NA. I think this means that there is missing data.
...
**Answer** January 1990 has a count of 12. Since this date is very basic I think it could mean that it's a placeholder date if they didn't have the data.

```{r}
yadkin_voters |> 
  mutate(registration_month = floor_date(registr_dt, "month")) |> ##floor shows totals for the month
...
**Answer** I would like to know more about the "deceased" or the "inactive" because it could mean the records are outdated.
...
**Answer** The amount field is a character
...
**Answer** There could be missing data, incomplete data, or outdated data
...
**Answer** It changed because R column names can't start with a number.
...
**Answer** It says: Error in `summarise()`:
ℹ In argument: `total_amount = sum(amount)`.
Caused by error in `sum()`:
! invalid 'type' (character) of argument
I think this means the sum() function is being applied to a column that has non-numeric data that is classified as character data.
...
**Answer** It is now a dbl or double
...
**Answer** There are two and the name is Derrick Hamilton
...
**Answer** The zip codes lost the +4 digits at the end.
...
I see two misspellings still. One has no "o" on the end and one has two.
...
**Answer** I still see the one with two os on the end.
...
**Answer** It looks like all of them are spelled correctly now.
