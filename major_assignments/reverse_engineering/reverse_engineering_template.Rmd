...
* **Analysis summary**: The reverse engineering produced the same findings as the report. My code found that 78.07% of Peter Franchot's contributions came from Maryland contributors.

```{r}
franchot_contribs <- read_csv("~/Documents/github-2024/GitHub/data_journalism_2024_fall/major_assignments/reverse_engineering/data/franchot_contribs.csv")

franchot_contribs <- franchot_contribs %>%
  mutate(state = str_extract(`Contributor Address`, "\\b[A-Z]{2}\\b"))

md_contributions <- franchot_contribs %>% 
  filter(state == "MD")

total_md_contributions <- sum(md_contributions$`Contribution Amount`, na.rm = TRUE)
total_contributions <- sum(franchot_contribs$`Contribution Amount`, na.rm = TRUE)

md_percentage <- (total_md_contributions / total_contributions) * 100
