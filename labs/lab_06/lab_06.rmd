...
```{r}
# Party registrations
party_registrations <- read_csv("~/Documents/github-2024/GitHub/data_journalism_2024_fall/labs/lab_06/data/new_registrations_by_party.csv")
party_registrations
```

```{r}
# Removals
removals <- read_csv("~/Documents/github-2024/GitHub/data_journalism_2024_fall/labs/lab_06/data/removals_by_party.csv")
...
```{r}
# Changes
changes <- read_csv("~/Documents/github-2024/GitHub/data_journalism_2024_fall/labs/lab_06/data/changes_in_registration.csv")
...
current_registrations <- read_csv("~/Documents/github-2024/GitHub/data_journalism_2024_fall/labs/lab_06/data/current_registrations_by_county.csv")
...
```{r}
# Party registrations
party_registrations_2022 <- read_csv("~/Documents/github-2024/GitHub/data_journalism_2024_fall/labs/lab_06/data/new_registrations_2022.csv")
...
```{r}
# Removals
removals_2022 <- read_csv("~/Documents/github-2024/GitHub/data_journalism_2024_fall/labs/lab_06/data/removals_2022.csv")
...
```{r}
# Changes
changes_2022 <- read_csv("~/Documents/github-2024/GitHub/data_journalism_2024_fall/labs/lab_06/data/changes_in_registration_2022.csv")
...
```{r}
# Current registrations
current_registrations_2022 <- read_csv("~/Documents/github-2024/GitHub/data_journalism_2024_fall/labs/lab_06/data/current_registrants_2022.csv")
...
A1. I would describe that republican party saw a small (less than one percent) decrease in registered voters while democrats saw a two percent decrease. The most interesting thing about these changes is that unaffiliated voters rose by over eight percent, that would be the lede.

```{r}
voter_data_2022 <- data_frame(
  party = c("DEM", "REP", "UNAF"),
  total_registered = c(2250961, 998905, 841573)
)

voter_data_2024 <- data_frame(
  party = c("DEM", "REP", "UNAF"),
  total_registered = c(2219417, 1009323, 923927)
)

total_voters_2022 <- sum(voter_data_2022$total_registered)
voter_data_2022$percentage_2022 <- (voter_data_2022$total_registered / total_voters_2022) * 100

total_voters_2024 <- sum(voter_data_2024$total_registered)
voter_data_2024$percentage_2024 <- (voter_data_2024$total_registered / total_voters_2024) * 100

merged_data <- merge(voter_data_2022, voter_data_2024, by = "party")

merged_data$percentage_change <- (merged_data$percentage_2024 - merged_data$percentage_2022) / merged_data$percentage_2022 * 100
...
A2. Baltimore County had an 83 percent increase, the largest change. Baltimore county voted for Biden in 2020 and Wes Moore in 2022. Baltimore City had the lowest percentage change with -11 percent. 

```{r}
library(dplyr)

voter_data <- data.frame(
  jurisdiction = c("BALTIMORE CO.", "MONTGOMERY", "PRINCE GEORGE", "ANNE ARUNDEL", "HOWARD", "BALTIMORE CITY", "FREDERICK", "HARFORD"),
  party = c("UNAF", "UNAF", "UNAF", "UNAF", "UNAF", "UNAF", "UNAF", "UNAF"),
  total_registered_2022 = c(108672, 160577, 86155, 98350, 57367, 63933, 54268, 45260),
  total_registered_2024 = c(199734, 173554, 100970, 105506, 63155, 56663, 49092, 41705)
)

voter_data$percentage_change <- (voter_data$total_registered_2024 - voter_data$total_registered_2022) / voter_data$total_registered_2022 * 100

voter_data_sorted <- voter_data %>%
...
A3. It did not match. The percentage change here for republicans is a 21% increase which is very significant. I think this is worthy of further explanation.

```{r}
new_registrants_2022 <- data_frame(
  party = c("DEM", "REP", "UNAF"),
  total_registered = c(19428, 5661, 11513)
)

new_registrants_2024 <- data_frame(
  party = c("DEM", "REP", "UNAF"),
  total_registered = c(19077, 6977, 10975)
)

total_registered_2022 <- sum(new_registrants_2022$total_registered)
total_registered_2024 <- sum(new_registrants_2024$total_registered)

new_registrants_2022 <- new_registrants_2022 %>%
  mutate(percentage_2022 = (total_registered / total_registered_2022) * 100)

new_registrants_2024 <- new_registrants_2024 %>%
  mutate(percentage_2024 = (total_registered / total_registered_2024) * 100)

merged_data <- merge(new_registrants_2022, new_registrants_2024, by = "party")
...
A4. My question: What are the percentage changes in the non-main parties and what trends stick out? My answer: There has been a significant decrease in members of the green party. If this trend continues this party will be very close to extinct here soon.

```{r}
voter_data_2022 <- data_frame(
  party = c("OTH", "LIB", "GRN"),
  total_registered = c(41709, 17591, 6565)
)

voter_data_2024 <- data_frame(
  party = c("OTH", "LIB", "GRN"),
  total_registered = c(48415, 19423, 4732)
)
...
A5. Why are more people choosing to identify as unaffiliated? This story would involve a lot of outside interviewing but could be an interesting story. From question one, there has been an eight percent increase in unaffiliated voters.
