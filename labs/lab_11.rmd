...
**A2** The most recent release with the word "campaign" in the title is from April, it is referring to the governor race and advocating for John King.

```{r}
press_releases_url <- "https://osp.maryland.gov/category/press-releases/"

press_releases <- read_html(press_releases_url)

press_releases <- press_releases |> 
  html_elements("li")

press_releases <- tibble(
  title = press_releases |> html_element("a") |> html_text(trim = TRUE),
  url = press_releases |> html_element("a") |> html_attr("href"),
  date = press_releases |> html_element("a") |> html_text(trim = TRUE) |> 
    str_extract("\\b\\d{4}-\\d{2}-\\d{2}\\b"))

press_releases <- press_releases |>
  separate(title, into = c("date", "clean_title"), sep = " - ", extra = "merge")
...
# Extract dates (update selector if necessary)
dates <- cardin_page |> 
  html_elements(".entry-date") |>  # Update selector for date
  html_text(trim = TRUE) |> 
  as_tibble()

# Combine columns into a single dataframe
press_releases_data <- bind_cols(
  date = dates$value,
  title = titles$value,
  url = urls$value
)
...
press_releases_data <- press_releases_data |>
  rename(Date = date, Title = title, URL = url) |>
  mutate(Date = mdy(Date))  # Convert date to Date type if in "MM/DD/YYYY" format

# View the final dataframe
print(press_releases_data)
```
